...
public class ShippingMethodServiceImpl implements ShippingMethodService{
	...
	public void insertShippingMethod(ShippingMethod shippingMethod) {
		shippingMethodDao.insertShippingMethod(shippingMethod);
		
	}
	
	@Override
	public void updateShippingMethod(ShippingMethod shippingMethod) {
		shippingMethodDao.updateShippingMethod(shippingMethod);
	}

	@Override
	public void deleteShippingMethod(ShippingMethod shippingMethod) {
		shippingMethodDao.deleteShippingMethod(shippingMethod);
		
	}

	@Override
	public void truncateShippingMethod() {
		shippingMethodDao.truncateShippingMethod();
	}
		...
		try{
			...
			for (int i = 0; i < list.size(); i++) {
				if(i != 0){
					...
					shippingMethod.setShippingMethodId(Long.parseLong(line[0]));
